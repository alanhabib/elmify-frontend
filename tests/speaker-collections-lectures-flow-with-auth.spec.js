...
test.beforeAll(async () => {
  ...
  // Enable testing mode
  console.log('🔧 Enabling authentication bypass for testing...');
  try {
    execSync('node enable-testing-mode.js true', { cwd: path.resolve('.') });
    console.log('✅ Authentication bypass enabled');
  } catch (error) {
    console.error('Failed to enable testing mode:', error.message);
    throw error;
  }
});
...
test.afterAll(async () => {
  ...
  console.log('🔧 Disabling authentication bypass...');
  try {
    execSync('node enable-testing-mode.js false', { cwd: path.resolve('.') });
    console.log('✅ Authentication bypass disabled');
  } catch (error) {
    console.error('Warning: Failed to disable testing mode:', error.message);
  }
});

test.describe('AudibleClone: Complete Flow with Auth Bypass', () => {
  let consoleMessages = [];
  let consoleErrors = [];
  let networkRequests = [];
  let testResults = {
  ...
  test('Complete speaker to lectures navigation flow with authentication bypass', async () => {
    ...
    try {
      ...
      console.log('Step 1: Loading initial page with auth bypass...');
      await page.goto(BASE_URL, { waitUntil: 'networkidle' });
      await page.waitForTimeout(3000); // Allow for initial data loading
      
      const step1Screenshot = path.join(SCREENSHOTS_DIR, '01-initial-page-load-bypass.png');
      await page.screenshot({ path: step1Screenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Initial Page Load (Auth Bypass)', path: step1Screenshot });
      testResults.steps.push({ step: 1, name: 'Initial Page Load with Auth Bypass', status: 'completed' });

      // Step 2: Verify we bypassed authentication (should be on main app, not sign-in)
      console.log('Step 2: Verifying authentication bypass...');
      const currentUrl = page.url();
      const isOnSignInPage = currentUrl.includes('sign-in') || currentUrl.includes('auth');
      
      if (isOnSignInPage) {
        throw new Error('Authentication bypass failed - still on sign-in page');
      }
      
      console.log(`✅ Successfully bypassed authentication. Current URL: ${currentUrl}`);
      testResults.steps.push({ step: 2, name: 'Authentication Bypass Verification', status: 'completed' });

      // Step 3: Wait for speakers to load and verify page content
      console.log('Step 3: Waiting for speakers to load...');
      ...
      const step3Screenshot = path.join(SCREENSHOTS_DIR, '02-speakers-loaded-bypass.png');
      await page.screenshot({ path: step3Screenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Speakers Loaded', path: step3Screenshot });
      testResults.steps.push({ step: 3, name: 'Speakers Data Loaded', status: 'completed' });

      // Step 4: Find and click on the first speaker
      console.log('Step 4: Finding and clicking first speaker...');
      ...
      const step4Screenshot = path.join(SCREENSHOTS_DIR, '03-speaker-modal-opened-bypass.png');
      await page.screenshot({ path: step4Screenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Speaker Modal Opened', path: step4Screenshot });
      testResults.steps.push({ step: 4, name: 'Speaker Modal Navigation', status: 'completed' });

      // Step 5: Find and click on a collection
      console.log('Step 5: Finding and clicking on collection...');
      ...
      if (collectionElement) {
        ...
        const debugScreenshot = path.join(SCREENSHOTS_DIR, '04-no-collections-found-debug-bypass.png');
        await page.screenshot({ path: debugScreenshot, fullPage: true });
      }

      const step5Screenshot = path.join(SCREENSHOTS_DIR, '04-collection-modal-opened-bypass.png');
      await page.screenshot({ path: step5Screenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Collection Modal Opened', path: step5Screenshot });
      testResults.steps.push({ step: 5, name: 'Collection Modal Navigation', status: 'completed' });

      // Step 6: Verify lectures are visible
      console.log('Step 6: Verifying lectures are visible...');
      ...
      const step6Screenshot = path.join(SCREENSHOTS_DIR, '05-lectures-view-bypass.png');
      await page.screenshot({ path: step6Screenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Lectures View', path: step6Screenshot });
      ...
      console.log('Step 7: Testing backend connectivity...');
      try {
      ...
      // Final screenshot
      const finalScreenshot = path.join(SCREENSHOTS_DIR, '06-final-state-bypass.png');
      await page.screenshot({ path: finalScreenshot, fullPage: true });
      testResults.screenshots.push({ step: 'Final State', path: finalScreenshot });

      testResults.success = true;
      console.log('🎉 Test completed successfully with authentication bypass!');
      ...
      const errorScreenshot = path.join(SCREENSHOTS_DIR, 'error-state-bypass.png');
  ...
  async function generateTestReport() {
    const report = `# AudibleClone End-to-End Test Report (With Auth Bypass)
...
- **Authentication:** ${testResults.authBypassEnabled ? '🔓 BYPASSED (Testing Mode)' : '🔐 ENABLED'}
...
- Testing environment properly configured` :
  `❌ **Authentication bypass not enabled**
- Normal authentication flow required
- May have been blocked by sign-in screen`
}

## Issues Identified
${testResults.consoleErrors.length === 0 && testResults.networkErrors.length === 0 ? 
  '🎉 No critical issues found! The application appears to be working correctly with authentication bypass.' :
...
3. **User Flow:** ${testResults.success ? 'Working correctly with auth bypass' : 'Navigation flow needs debugging'}
4. **Authentication:** Authentication bypass is working correctly for testing

---
*Generated automatically by Playwright test with authentication bypass at ${new Date().toISOString()}*
`;
